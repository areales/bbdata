...
**As of 2026-08-29: the P1 wave is closed — all 14 P1-series items fixed this session (unreleased — see "Unreleased" section).**
Remaining open work: P2.7, P3.5, P3.6, and the P4.6–P4.11 backlog. Three
course-side follow-ups accumulated: the `trend-year-over-year` lesson command
needs `--stat pitching` appended; the matchup teammate example (already filed
course-side); and the m03-l04 `--validate` teaching should be re-checked now
that the `required-data` check makes `--no-strict --validate` fail on hollow
reports.
