...
- **Course-side follow-ups.** Most of these were closed 2026-09-07 in
  ...
  leverage + `--stat pitching`, the User Guide template rows, the M03/M04
  drift. **Still open:** the m03-l04 `--validate` teaching now inverts, since
  `--no-strict --validate` fails on hollow reports on 0.11 (filed as
  `../video-hyperframes/TASKS.md` #69 item 6); and lesson annotations quoting
  `N = 386` batted balls will drift on re-capture, because P3.6 retains
...
1. ~~**The percent-formatting family — P1.4, P1.5, P1.6.**~~ **Fixed 2026-08-29
   (shipped in v0.11.0).** Shared `fmtPercent` in `src/utils/stat-format.ts`; formatters
   no longer guess — percent rendering requires a template-declared
   `columnFormats()` hint. See the "Shipped in v0.11.0" section.
2. ~~**Wrong quantity, not wrong scale — P1.10, P1.17.**~~ **Fixed 2026-08-29
   (shipped in v0.11.0).** One root cause as predicted: the stat lookup stripped `%`/`+`
   from both sides, colliding `BB%` with `BB` and `wRC+` with `wRC`. See the
   "Shipped in v0.11.0" section.
3. ~~**Fabricated and mislabeled values — P1.11, P1.16.**~~ **Fixed 2026-08-29
   (shipped in v0.11.0).** The six tracking columns are nullable end-to-end (schema +
   adapter + every consumer), and arsenal break renders in true inches (×12).
   See the "Shipped in v0.11.0" section.
4. ~~**Silent wrong answers — P1.15, P1.7.**~~ **Fixed 2026-08-29 (shipped in v0.11.0).**
   ...
   rate. See the "Shipped in v0.11.0" section.
5. **Zero-row failures on the course's own printed examples — P1.8, P1.9, ~~P1.12~~.**
   ...
   **All three fixed 2026-08-29 (shipped in v0.11.0)** — stat aliases + actionable
...
## Shipped in v0.11.0 — fixed 2026-08-29, released 2026-09-07
...
- **P4.12 — viz fixture gallery covers all 6 chart types.** Decision taken: add both fixtures. `movement-binned` renders from `season-pitches.sample.json` — the former orphan, regenerated with realistic per-type scatter (the old file's 576 rows had zero variance: every FF carried identical `pfx` values, which a binned density chart renders as three lone cells; git history shows it came from the auto-commit era with no consumer). `pitcher-rolling` gets a new `pitcher-rolling-windows.sample.json` with 8 five-start windows in the wide string shape `pitcher-rolling-trend` emits (`"96.2 mph"`, `"29.4%"`). `scripts/render-fixtures.ts` now exports `FIXTURES` and guards `main()` behind a direct-run check so the new drift test — `test/scripts/render-fixtures.test.ts`, asserting every `listChartTypes()` entry has a gallery row whose fixture file exists — can import the list without triggering a render. Both PNGs visually verified: three distinct density clusters with mean crosses; four faceted metric panels with dip-and-recovery trend lines.
- **P4.13 — `report --help` template list generated from the registry.** New `formatReportTemplateList()` in `src/commands/report.ts`, mirroring G.1's `formatTemplateList()`: buckets `getAllReportTemplates()` by category with a `Record<ReportCategory, string>` label map (compiler forces a label when a new category is added) and renders in stable order. The `.addHelpText` block is now a lazy function so the list reflects the registry at render time. Regression: `test/commands/report-help.test.ts` (2 tests — full enumeration ≥13 templates + stable category order). Full gate: 405/405, typecheck and lint clean.
