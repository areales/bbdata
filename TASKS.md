...
**As of 2026-08-29 (evening): 7 of the 14 filed P1 items are fixed on branch
`claude/easy-tasks-review-689f49`, unreleased — P1.4, P1.5, P1.10, P1.11, P1.14,
P1.16, P1.17, plus P4.9.** Detail in the "Fixed on branch — unreleased" section
below; every fix was verified against the live repro command from its entry.
**7 P1 items remain pending: P1.6, P1.7, P1.8, P1.9, P1.12, P1.13, P1.15.**
Every entry carries a reproduction command and, where diagnosed, the source line.
...
1. **The percent-formatting family — P1.4 ✅, P1.5 ✅ (fixed 2026-08-29), P1.6.** `fmtPercent` is
   duplicated across two query templates (×100 on values already ratios) and the
   two formatters guess percentages from magnitude. One shared, typed helper
   closes all three. P1.6 is the big one: it corrupts any value in (−1, 1),
   including xwOBA and grid indices, in every human-facing table.
2. **Wrong quantity, not wrong scale — P1.10 ✅, P1.17 ✅ (fixed 2026-08-29, one shared root cause).** `leaderboard-comparison`
   returns walk *totals* under a `BB%` label, and its wRC+ disagrees with
   `hitter-season-profile` by 41 points for the same player. Same template; likely
   one transform.
3. **Fabricated and mislabeled values — P1.11 ✅, P1.16 ✅ (fixed 2026-08-29).** `savant-csv` zero-fills
   six missing columns (a dropped spin rate becomes a measured 0 rpm), and
   `pitcher-arsenal` labels feet as inches. Both XS, both reach students.
4. **Silent wrong answers — P1.15, P1.7.** `--pitch-type` is ignored without a
   warning on `pitcher-raw-pitches`; `pitcher-arsenal`'s whiff denominator omits
...
## Fixed on branch — unreleased (2026-08-29, `claude/easy-tasks-review-689f49`)

Seven facts-gate defects plus the test-plan wording drift, fixed in one pass and
verified three ways: full gates green (`typecheck` / `lint` / 334 tests, up from
309), plus a live re-run of each entry's original repro command. Not yet
released — fold into the next version's CHANGELOG at release time.

- **P1.4 + P1.5 — `fmtPercent` ×100 (3 metrics, 2 templates).** The duplicated
  helper is hoisted to `src/utils/format.ts` with a normalize guard
  (`Math.abs(v) <= 1 ? v * 100 : v`) so it survives FanGraphs switching between
  ratio and pre-scaled representation. Both season-profile templates import it;
  the local copies are gone. Tests: `test/utils/format.test.ts` (5 cases) +
  ratio-form regressions in `test/templates/season-profile.test.ts`. The old
  tests fed pre-scaled fixtures (`'K-BB%': 18.5`), which is why the bug never
  tripped them — the new cases feed the live ratio shape.
- **P1.10 + P1.17 — one lookup bug, two filed symptoms.** `findStatValue()` in
  `leaderboard-comparison` stripped `+`/`%` before matching, so `BB%` → `bb`
  collided with FanGraphs' `BB` walk-total column and `wRC+` → `wrc` collided
  with `wRC` (weighted runs created — Judge's "162.8 wRC+" was his wRC all
  along). Exact-key match now runs before the normalized fallback. New suite:
  `test/templates/leaderboard-comparison.test.ts` (5 cases).
- **P1.11 — `savant-csv` fabricated zeros.** All six tracking columns
  (`release_speed`, `release_spin_rate`, `pfx_x`, `pfx_z`, `plate_x`, `plate_z`)
  route through the existing `numOrNull` helper; blank Statcast cells reach the
  `{ data, meta }` envelope as `null`. Real scope was S, not XS: the six fields
  went `.nullable()` in `PitchDataSchema`, and every consumer that did
  arithmetic on them needed a null-guard — `pitcher-arsenal` means now skip
  null fields (em-dash when a whole group is untracked), the two zone templates
  skip location-untracked pitches, the two velocity trends filter null speeds,
  and both movement charts drop null-pfx points instead of plotting them at the
  origin. Regressions: blank-cell case in `test/adapters/savant.test.ts`,
  null-mean cases in `test/templates/pitcher-arsenal.test.ts`.
- **P1.14 — UTC report dates.** `src/commands/report.ts` stamps
  `new Date().toLocaleDateString('en-CA')` (same `YYYY-MM-DD` shape, local
  time). Evening runs no longer claim tomorrow's date.
- **P1.16 — arsenal break labeled inches, valued in feet.** `H Break` /
  `V Break` means scale ×12 at format time, matching the `movement` chart's
  existing `feet → inches` conversion. The `pfx_*` comments in
  `src/adapters/types.ts`, which wrongly said "(inches)", now say feet.
  Regression: inches case in `test/templates/pitcher-arsenal.test.ts`.
- **P4.9 — COURSE_TEST_PLAN wording.** R.7 expects the template's actual
  "Times Through the Order" heading; F.22 asserts viridis as `rgb(…)` triplets;
  F.19 accepts the R2.1 config-gate error as a third valid outcome.

**Course-side note:** the P1.4/P1.5/P1.16 corrections change numbers that
lesson artifacts previously had to route around; the facts gates that filed
these entries (`m01-l02-v2`, `m01-l03`, `m04-l02`, `m04-l03`) can re-admit
K-BB%/BB%/K% and break magnitudes once this ships.
