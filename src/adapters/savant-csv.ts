...
export function parseSavantCsv(csvText: string): PitchData[] {
  ...
  return filteredRows.map((row) => ({
    ...
    // Tracking dropouts leave these blank; `Number('') === 0` would turn a
    // missing spin rate into a measured 0 rpm (P1.11), so preserve null.
    release_speed: numOrNull(row.release_speed),
    release_spin_rate: numOrNull(row.release_spin_rate),
    pfx_x: numOrNull(row.pfx_x),
    pfx_z: numOrNull(row.pfx_z),
    plate_x: numOrNull(row.plate_x),
    plate_z: numOrNull(row.plate_z),
