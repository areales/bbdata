...
// Register the shared footer partial once at module load so every report
// template can `{{> footer}}` to render the CLI version, data sources, date,
// and AI-assistance disclaimer. Keeps cliVersion in sync with package.json
// instead of duplicating version strings across 13 templates.
...
export async function report(options: ReportOptions): Promise<ReportResult> {
  ...
  const rawContent = compiled({
    ...
    season,
    audience,
    // Local date, not toISOString() — UTC would stamp evening runs with
