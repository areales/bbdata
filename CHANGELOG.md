...
bug fixes (`--help` enumeration and `--pitch-type` on stdin), a long-
orphaned footer partial finally wired through every report template, and
a standing test plan (`COURSE_TEST_PLAN.md`) that replaces the drifting
versioned `TEST_PLAN.md`. No breaking changes to the CLI surface or the
programmatic API. The vega/vega-lite major bump below is transparent to
CLI users — emitted SVG / PNG / PDF is byte-identical for the 5
pre-existing canonical chart types.
...
- **Reports now carry a `bbdata` version footer.** `src/templates/reports/partials/footer.hbs`
  existed with `{{cliVersion}}` and an AI-assistance disclaimer, but the
  partial was never registered with Handlebars and no `.hbs` template
  referenced it — every report instead emitted a hardcoded
  "Generated by bbdata CLI …" line that omitted the version entirely.
  `src/commands/report.ts` now registers the partial once at module load
  and all 13 report templates plus `generateFallbackTemplate` use
  `{{> footer}}`. Regression test at `test/commands/report.test.ts` reads
  the version from `package.json` at test time and asserts it appears in
  the rendered output, so a future refactor can't silently re-orphan the
  partial. A companion `scripts/lint-partials.ts` script (wired into
  `prepublishOnly` via a new `lint:partials` npm script) walks every
  `src/templates/reports/partials/*.hbs` file and fails the publish gate
  if any partial is unreferenced by a rendered template.
- **G.1 — `bbdata query --help` enumerates all registered templates.**
  The previous hardcoded help string listed 12 of 21 shipped templates;
  9 bonus templates added across v0.7.x–v0.9.0 (`pitcher-raw-pitches`,
  `pitcher-recent-form`, `pitcher-by-count`, `pitcher-tto`,
  `pitcher-season-profile`, `hitter-zone-grid`, `hitter-raw-bip`,
  `hitter-season-profile`, `hitter-handedness-splits`) were invisible to
  students discovering the CLI via `--help`. New `formatTemplateList()`
  helper in `src/commands/query.ts` buckets the live registry by
  `QueryCategory` at help-render time (wrapped in a thunk so side-effect
  template registration has completed by the time Commander renders the
  help text). With F1.1's `pitcher-rolling-trend` added in this release,
  `--help` now enumerates all 22. Regression coverage in
  `test/commands/query-help.test.ts` calls `formatTemplateList()`
  directly — avoiding a Commander `helpInformation()` quirk where
  programmatic access skips `addHelpText` callbacks — and asserts the
  output count matches the live registry so any new template
  automatically participates without a docs edit.
- **G.7 — `--pitch-type` filter works with `--data` and `--stdin`.**
  Previously the flag was silently a no-op on the stdin path: passing
  `--pitch-type FF --data fixture.csv` returned every pitch type in the
  payload. Network adapters (Savant, MLB Stats API) honored the filter;
  the stdin adapter didn't apply it before returning records.
  `src/adapters/stdin.ts` now applies `query.pitch_type` as a
  case-insensitive `Set` to pitch-level records before returning and
  no-ops on `PlayerStats` rows (which don't carry a `pitch_type` field).
...
- **`COURSE_TEST_PLAN.md` replaces `TEST_PLAN.md`.** The prior plan was
  organized by version — which forced maintainers to archaeology their
  way through `0.7.0 / 0.7.1 / 0.7.2 / 0.8.0 / 0.9.0` sections to answer
  "is this flag still covered?" and had already drifted (the v0.9.0
  section referenced a `fixtures/burnes-2025.json` that never existed).
  The new plan is organized by **CLI surface** — commands, templates,
  flags — and cross-references every `bbdata` claim in
  `../ai-baseball-data-analyst/.claude/skills/*/SKILL.md`, the course
  deliverables, and the User Guide against the live CLI. 8 sections,
  ~100 rows, each tagged **C** (Claude-automatable structural check) or
  **A** (Aaron-only — perceptual / live-network). Release smoke is now
  a subset of COURSE_TEST_PLAN rows re-run against the sections a given
  release touches, documented in `CLAUDE.md` and step 6 of
  `.claude/commands/release-preflight.md`. G.7 above was discovered via
  a COURSE_TEST_PLAN row on its first pass.
- **`prepublishOnly` gained a `lint:partials` step.** New
  `scripts/lint-partials.ts` walks every `.hbs` file under
  `src/templates/reports/` and fails if a partial in `partials/` is not
  referenced by any rendered template (the exact class of bug that hid
  the footer partial). Runs between `lint` and `typecheck` in the
  publish gate.
