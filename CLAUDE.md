...
**Every release also re-runs the `COURSE_TEST_PLAN.md` sections affected by the release.** The plan is a standing contract against the course material's bbdata claims (skills, deliverables, User Guide), organized by CLI surface (commands / templates / flags) not by version. For a release, identify the sections that changed — e.g., a new flag on `viz` means re-run §4A and §5; a new query template means re-run §2 and §1 (`--help` enumeration). Live-network "A" rows are optional on patch releases; required on minor/major.

**Every release also reconciles `TASKS.md`.** Before `npm version`, move any items completed this cycle from the Priority 1–4 sections into a "Shipped in vX.Y.Z" section at the top, with one-line evidence pointers (`file:line` or commit hash). The release flow already forces you to touch CHANGELOG.md; TASKS.md needs the same discipline or the backlog drifts silently across releases (v0.7.0 and v0.7.1 both shipped without crossing off the nine items they completed).
